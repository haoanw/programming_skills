...
The Standards axis needs nothing. It reads whatever the repo documents (`CODING_STANDARDS.md`, `CONTRIBUTING.md`, and the like), applies [clean-code](https://aihero.dev/skills-clean-code) as its default code-quality baseline and [clean-architecture](https://aihero.dev/skills-clean-architecture) when the diff touches boundaries, dependency direction or external mechanisms, and falls back on a built-in baseline when the repo documents nothing.
...
| Reads | The repo's documented standards, plus the clean-code, relevant clean-architecture and smell baselines | The originating issue or spec |
