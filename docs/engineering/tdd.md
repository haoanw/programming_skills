...
[codebase-design](https://aihero.dev/skills-codebase-design) needs to be installed. `tdd` used to carry its own deep-module and interface-design notes; in v1.0 those were deleted in favour of the shared skill, and `tdd` now leans on it for interface-design vocabulary. It also applies [clean-code](https://aihero.dev/skills-clean-code) to each green implementation and its tests, and pulls in [clean-architecture](https://aihero.dev/skills-clean-architecture) only when a slice changes a dependency or module boundary; neither is permission to build beyond the current behaviour slice. Nothing else; the skill is [stateless](https://www.aihero.dev/ai-coding-dictionary/stateless) and writes no files of its own.
