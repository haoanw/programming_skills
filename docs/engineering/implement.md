...
Around that core it keeps the loop tight — typecheck often, run single test files as it goes, run the whole suite once at the end. It applies [clean-code](https://aihero.dev/skills-clean-code) throughout production and test code, and pulls in [clean-architecture](https://aihero.dev/skills-clean-architecture) when the work changes boundaries, dependencies, policy placement, adapters, or composition. Repository standards and the settled spec still win over either heuristic. It then closes out with a review pass and a commit to the current branch.
...
Reach for it after the work has been specced and sequenced, not before. Its key neighbours are [to-tickets](https://aihero.dev/skills-to-tickets), which produces the tickets — each declaring its blocking edges — that it works through, and [tdd](https://aihero.dev/skills-tdd), which it drives internally to write the tests at each seam before running its own [code-review](https://aihero.dev/skills-code-review) pass with the clean-code and relevant clean-architecture baselines, then committing. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
